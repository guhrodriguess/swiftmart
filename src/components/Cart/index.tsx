    ...
    <>
      ...
      >
        <Styles.Info>
          ...
          <div>
            {cartItems.length > 0 && (
              <button onClick={handleRemoveAllItems}>Limpar tudo</button>
            )}
          </div>
        </Styles.Info>
        ...
        {cartItems.length > 0 && (
          <Link to="/purchase">
            <button id="buy" onClick={handleCloseCartBar}>
              finalizar compra <ArrowRight weight="bold" id="arrowRight" />
            </button>
          </Link>
        )}
      </Styles.CartWrapper>

      {isCartVisible && <Styles.PageOpacity onClick={handleCloseCartBar} />}
    </>
  );
}
